...
(function(window, undefined) {
	var D = MingGe,
		...
		defaultArg, del = function(eve) {
			var container = this,
				MGSL, cache, isUn = isUndefined(eve = trim(eve)),
				count, isWin, eCache, elem, bodys = DOC.body;
	...
	scrollLoad.prototype = {
		...
		handle: function(container, elem, arg, eCache, isScroll, isForce) {
			var MGSL = eCache[MG],
				cache, isCache = isNumber(MGSL) && (cache = cacheData[MGSL]);
		...
		writeEvent: function(callback, elem, arg) {
			...
			if (arg.container && arg.container.version) {
				while (container = arg.container[i++]) {
		...
		Calculation: function(sL, sT, cw, ch) {
			var img = this.img,
				i = img.length - 1;
			if (i < 0) {
				this.arg.del && del.call(this.arg.container, this.arg.event);
			} else {
				for (; i > -1; i--) {
	...
	D.fn.extend({
		...
		scrollLoad: function(options, func) {
			new scrollLoad(this.slice(), options, func);
			return this;
		}
	});
})(window);